$(document).ready(function(){
    $("#form6").on('submit', function(event){
        ...
        // Verificar si algún checkbox en el grupo "deportes" está seleccionado
        if(!$("input[name='deportes']:checked").length) {
            // Aplicar estilo de error a los checkboxes
            $("input[name='deportes']").each(function() {
                $(this).css({
                    "box-shadow": "0 0 5px 1px red" // Aplicar box-shadow para error
                });
            });
            ...
            $("label[for='deportes-practica']").after($aviso);

            verificaDeportes = false;
        } else {
            // Restablecer el estilo de los checkboxes si se selecciona al menos uno
            $("input[name='deportes']").each(function() {
                $(this).css({
                    "box-shadow": "" // Restablecer box-shadow
                });
        ...
        if(!verificaNombreApellido || !verificaEdad || !verificaDireccion || !verificaEstudios || !verificaSexo || !verificaDeportes){
